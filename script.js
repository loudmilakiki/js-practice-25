...
async function postDataWithAuth(url, data, authToken) {
  try {
    ...
    if (dataAuth.ok) {
      const result = await dataAuth.json();
...
// Вказуємо заголовок (headers) "Content-Type" зі значенням "application/json"
...
async function displayData() {
  ...
  let data = await dataGenerator.next();
  console.log(data.value);
  data = await dataGenerator.next();
  console.log(data.value);
  data = await dataGenerator.next();
  console.log(data.value);
}
